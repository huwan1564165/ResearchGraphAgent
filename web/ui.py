...
<script>
const form=document.querySelector('#project'), workflow=document.querySelector('#workflow'), questionsBox=document.querySelector('#questions'), statusBox=document.querySelector('#status'), runButton=document.querySelector('#run'); let projectId, questions=[];
...
form.onsubmit=async event=>{event.preventDefault();const submit=form.querySelector('button');submit.disabled=true;setStatus('正在生成子问题，请稍候。');try{const data=Object.fromEntries(new FormData(form));const created=await api('/api/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});projectId=created.project.id;await api(`/api/projects/${projectId}/questions`,{method:'POST'});const result=await api(`/api/projects/${projectId}/questions`);questions=result.questions;renderQuestions();workflow.classList.remove('hidden');setStatus('子问题已生成，可编辑、删除或新增。');}catch(error){setStatus(error.message,true)}finally{submit.disabled=false}};
